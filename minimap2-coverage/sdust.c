...
int main(int argc, char *argv[])
{
	...
	while (kseq_read(ks) >= 0) {
		uint64_t *r;
		uint32_t masked_length = 0;
		int i, n;
		r = sdust(0, (uint8_t*)ks->seq.s, -1, T, W, &n);
		for (i = 0; i < n; ++i)
		    masked_length += (int)r[i] - (int)(r[i]>>32);
		    //printf("%s\t%d\t%d\n", ks->name.s, (int)(r[i]>>32), (int)r[i]);
		printf("%s\t%d\t%d\t%.3f\t%.3f\t%d\n",
		       ks->name.s,
		       masked_length, ks->seq.l,
		       (double)masked_length/ks->seq.l,
		       meanQ(ks->qual.s, ks->qual.l),
		       getQV(ks->qual.s, 7, ks->qual.l)
		       );
		free(r);
	}
	kseq_destroy(ks);
