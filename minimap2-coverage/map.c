								  ...
								  int *n_mini_pos, uint64_t **mini_pos)
{
	...
	*mini_pos = (uint64_t*)kmalloc(km, mv->n * sizeof(uint64_t));
    ...
    // YF memo: loop over each minimizer on a query.
	for (i = 0, *n_a = 0; i < mv->n; ++i) // find the length of a[]
		if (m[i].n < max_occ) *n_a += m[i].n; // YF memo: the number of non-repetitive minimizers' hit occurence.
	a = (mm128_t*)kmalloc(km, *n_a * sizeof(mm128_t));
	for (i = *rep_len = 0, *n_a = 0; i < mv->n; ++i) {
		mm128_t *p = &mv->a[i];
		mm_match_t *q = &m[i];
		const uint64_t *r = q->cr; // coordinates of i-th minimizer
...
void mm_map_frag(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, const char **quals, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	if (n_segs == 1) { // uni-segment
		regs0 = align_regs(opt, mi, b->km, qlens[0], seqs[0], quals? quals[0] : 0, &n_regs0, regs0, a); // YF memo: within this funcion, alignment is called.
